...
// CONFIGURATION IMPORTANTE - Modifiez ces valeurs
const START_INDEX = 20;  // Carte de départ (20)
const END_INDEX = 38;    // Carte de fin (38)
...
const toutesLesCartes = [
  ...
  },
  {
    "id": 20,
    "name": "Nightsister Bloodmage",
    "illustration": "Nightsister aux tatouages noirs, entourée d'une aura rouge sombre et énergie mystique en lévitation. Elle manipule de l'énergie rouge flottante formant des formes tourbillonnantes. Cimetière dathomiri en fond."
  },
  {
    "id": 21,
    "name": "Wookie Champion",
  ...
  // ... (les autres cartes continuent mais ne seront pas générées)
];

// Filtrer pour obtenir seulement les cartes demandées (20 à 38 inclus)
const cartes = toutesLesCartes.filter(card => card.id >= START_INDEX && card.id <= END_INDEX);

console.log(`\n🎯 Génération des cartes ${START_INDEX} à ${END_INDEX}`);
console.log(`📊 Nombre de cartes à générer: ${cartes.length}\n`);
...
async function generateRangeImages() {
  const apiKey = process.env.API_KEY;
  ...
  for (let i = 0; i < cartes.length; i++) {
    ...
    try {
      console.log(`\n[${i + 1}/${cartes.length}] Traitement de: ${card.name} (ID: ${card.id})`);
  ...
  console.log('📊 RÉSUMÉ DE LA GÉNÉRATION');
  console.log('='.repeat(50));
  console.log(`🎯 Cartes ciblées: ${START_INDEX} à ${END_INDEX}`);
  console.log(`✅ Succès: ${results.success.length}/${cartes.length}`);
  console.log(`❌ Échecs: ${results.failed.length}/${cartes.length}`);
  ...
  if (results.failed.length > 0) {
    console.log('\n❌ Cartes échouées:');
    results.failed.forEach(({ card, error }) => {
      console.log(`   - ${card.name} (ID: ${card.id}): ${error}`);
    });
  }
...
generateRangeImages().catch(console.error);