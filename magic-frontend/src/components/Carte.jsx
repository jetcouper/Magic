...
export default function Carte({ onClick, className = "", credit = "", life = "", nom = "", description = "", attack = "", imageURL = "", state = "" }) {
    ...
    // map state prop to holo color class
    const stateToClass = {
        "IDLE": 'holo-cyan',
        selected: 'holo-green',
        "SLEEP": 'holo-red',
        buffed: 'holo-green',
        exhausted: 'holo-yellow'
    };

    const holoClass = stateToClass[state] || 'holo-cyan';
    ...
    return (
        ...
        >
            {/* holo overlay depends on state */}
            <div className={`holo-overlay ${holoClass} rounded absolute pointer-events-none`}></div>
